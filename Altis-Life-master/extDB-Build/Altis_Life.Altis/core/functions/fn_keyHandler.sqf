...
switch (_code) do {
	...
	case 57: {
		...
		if(isNil "jumpCoolDown") then {jumpCoolDown = false;};
		if(life_is_arrested or (player GVAR "restrained")) then {
		_handled = true;
		};
		if(_shift && jumpCoolDown) exitWith {hintSilent "Du kannst nur alle 5 Sekunden springen";};
		jumpCoolDown = true;
